...
class SlideshowController extends BaseController {
	...
	public function lists() {
		$has_slide = 1;
		$file = ONETHINK_ADDON_PATH . MODULE_NAME . '/View/TemplateIndex/' . $this->config ['template_index'] . '/info.php';
		if (file_exists ( $file )) {
			$info = require_once $file;
			if (isset ( $info ['has_slide'] ) && $info ['has_slide'] == 0) {
				$has_slide = 0;
			}
		}else if(file_exists ( ONETHINK_ADDON_PATH . 'WeiSite/View/pigcms/Index_' . $this->config ['template_index'] . '.html' )){
	...
	public function edit() {
		...
		$data || $this->error( '400494:数据不存在！' );
		
		$token = get_token ();
		if (isset ( $data ['token'] ) && $token != $data ['token'] && defined ( 'ADDON_PUBLIC_PATH' )) {
			$this->error( '400495:非法访问！' );
		}
		...
		if (IS_POST) {
			...
			if ($Model->create () && $Model->save ()) {
				...
				$this->error( '400496:'. $Model->getError () );
			}
		} else {
