...
class FooterController extends BaseController {
	...
	public function lists() {
		$has_data = 1;
		$file = ONETHINK_ADDON_PATH . MODULE_NAME . '/View/TemplateFooter/' . $this->config ['template_footer'] . '/info.php';
		if (file_exists ( $file )) {
	...
	public function edit() {
		...
		if (IS_POST) {
			...
			$data || $this->error( '400489:数据不存在！' );
			
			$token = get_token ();
			if (isset ( $data ['token'] ) && $token != $data ['token'] && defined ( 'ADDON_PUBLIC_PATH' )) {
				$this->error( '400490:非法访问！' );
			}
	...
	public function add() {
		...
		if (IS_POST) {
			...
			if ($Model->create () && $id = $Model->add ()) {
				...
				$this->error( '400491:'. $Model->getError () );
			}
		} else {
			// 获取一级菜单
			$map ['pid'] = 0;
