...
class WapController extends WapBaseController {
	...
	function index() {
		...
		if (file_exists ( ONETHINK_ADDON_PATH . 'WeiSite/View/pigcms/Index_' . $this->config ['template_index'] . '.html' )) {
			$this->pigcms_index ();
			$this->display ( ONETHINK_ADDON_PATH . 'WeiSite/View/pigcms/Index_' . $this->config ['template_index'] . '.html' );
		} else {
			...
			$this->_footer ();
			// $backgroundimg=ONETHINK_ADDON_PATH.'WeiSite/View/TemplateIndex/'.$this->config['template_index'].'/icon.png';
			...
			// dump ( $html );
			$this->display ( ONETHINK_ADDON_PATH . 'WeiSite/View/TemplateIndex/' . $html . '/index.html' );
		}
	}
	// 分类列表
	function lists() {
		...
		if (file_exists ( ONETHINK_ADDON_PATH . 'WeiSite/View/pigcms/Index_' . $this->config ['template_lists'] . '.html' )) {
			
			$this->pigcms_lists ( $cate_id );
			$this->display ( ONETHINK_ADDON_PATH . 'WeiSite/View/pigcms/Index_' . $this->config ['template_lists'] . '.html' );
		} else {
			$map ['token'] = get_token ();
			if ($cate_id) {
				$map ['cate_id'] = $cate_id;
			...
			if (! empty ( $category )) {
				...
				$this->display ( ONETHINK_ADDON_PATH . 'WeiSite/View/TemplateSubcate/' . $cate ['template'] . '/' . $htmlstr );
			} else {
				...
				$this->display ( ONETHINK_ADDON_PATH . 'WeiSite/View/TemplateLists/' . $html . '/lists.html' );
			}
		}
	}
	// 详情
	function detail() {
		if (file_exists ( ONETHINK_ADDON_PATH . 'WeiSite/View/pigcms/Index_' . $this->config ['template_detail'] . '.html' )) {
			$this->pigcms_detail ();
			$this->display ( ONETHINK_ADDON_PATH . 'WeiSite/View/pigcms/Index_' . $this->config ['template_detail'] . '.html' );
		} else {
			...
			$this->_footer ();
			$this->display ( ONETHINK_ADDON_PATH . 'WeiSite/View/TemplateDetail/' . $this->config ['template_detail'] . '/detail.html' );
		}
	}
	...
	function _footer($temp_type = 'weiphp') {
		if ($temp_type == 'pigcms') {
			...
			$html = $this->fetch ( ONETHINK_ADDON_PATH . 'WeiSite/View/TemplateFooter/' . $this->config ['template_footer'] . '/footer.html' );
	 ...
	 * 5、把pigcms 目录下所有文档代码里的 Wap/Index/lists 替换成 WeiSite/Wap/lists
	 * 6、把pigcms 目录下所有文档代码里的 Wap/Index/index 替换成 WeiSite/Wap/index
	 */
	function pigcms_init() {
		...
		define ( 'RES', ONETHINK_ADDON_PATH . 'WeiSite/View/pigcms/common' );
