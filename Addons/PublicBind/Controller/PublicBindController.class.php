...
class PublicBindController extends ManageBaseController {
	function setTicket() {
		...
		if ($errCode == 0) {
			...
			if (isset ( $data ['ComponentVerifyTicket'] )) {
				...
				$map2 ['mp_id'] = M ( 'apps' )->where ( $map )->getField ( 'id' );
				if ($map2 ['mp_id']) {
					$uids = M ( 'apps_link' )->where ( $map2 )->getFields ( 'uid' );
					
					M ( 'apps' )->where ( $map )->limit ( 1 )->delete ();
					M ( 'apps_link' )->where ( $map2 )->delete ();
	...
	function after_auth() {
		...
		$data ['is_bind'] = 1;
		$data ['encodingaeskey'] = C ( 'ENCODING_AES_KEY' );
		$data ['cTime'] = NOW_TIME;
