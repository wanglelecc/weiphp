...
class UserCenterController extends ManageBaseController {
	var $syc_wechat = true;
	...
	public function lists() {
		...
		if ($isDown) {
			...
			$data = M ()->table ( $px . 'apps_follow as f' )->join ( $px . 'user as u ON f.uid=u.uid' )->field ( 'u.uid,f.openid' )->where ( $map )->order ( $order )->page ( $page, $row )->select ();
			/* 查询记录总数 */
	...
	function set_login() {
		...
		$data || $this->error ( '400411:数据不存在！' );
		
		if (IS_POST) {
			if (empty ( $_POST ['nickname'] ) || empty ( $_POST ['login_password'] )) {
				$this->error ( '400412:账号信息不能为空' );
			}
			
			$save ['nickname'] = I ( 'nickname' );
			$old_uid = M ( 'user' )->where ( $save )->getField ( 'uid' );
			if ($old_uid > 0 && $old_uid != $id) {
				$this->error ( '400413:该账号已经存在，请更换后再试' );
			}
	...
	public function edit() {
		...
		if (IS_POST) {
			...
			if ($Model->create () && $Model->where ( $map )->save ()) {
				...
				// lastsql();
				// dump($map);exit;
				$this->error ( '400415:' . $Model->getError () );
			}
		} else {
			$fields = get_model_attribute ( $model ['id'] );
			// dump($fields);
	...
	public function changeGroup() {
		...
		if (empty ( $uids )) {
			$this->error ( '400417:请选择用户!' );
		}
		$group_id = I ( 'group_id', 0 );
		if (empty ( $group_id )) {
			$this->error ( '400418:请选择用户组!' );
		}
	...
	public function changeTag() {
		...
		if (empty ( $uids )) {
			$this->error ( '400419:请选择用户!' );
		}
		$tags = array_filter ( explode ( ',', I ( 'tags' ) ) );
		if (empty ( $tags )) {
			$this->error ( '400420:请选择用户标签!' );
		}
		$map ['uid'] = array (
				'in',
				$uids 
	...
	// 第一步：获取全部用户的ID，并先保存到apps_follow表中，新的用户UID暂时为0，后面的步骤补充
	function syc_openid() {
		$map ['token'] = $save ['token'] = get_token ();
		
		$next_openid = I ( 'next_openid' );
		if (! $next_openid) {
			$res = M ( 'apps_follow' )->where ( $map )->setField ( 'has_subscribe', 0 );
		}
		...
		$res = M ( 'apps_follow' )->where ( $map )->save ( $pdata );
		if ($res != $data ['count']) {
			// 更新的数量不一致，可能有增加的用户openid
			$openids = ( array ) M ( 'apps_follow' )->where ( $map )->getFields ( 'openid' );
	...
	function syc_user() {
		...
		$map ['syc_status'] = 0;
		$map ['has_subscribe'] = 1;
		$list = M ( 'apps_follow' )->where ( $map )->field ( 'uid,openid' )->limit ( 100 )->select ();
		...
		// M ( 'apps_follow' )->where ( $map2 )->setField ( 'has_subscribe', 0 );
		...
		if ($countdata ['list_count'] != $countdata ['wp_data_count']) {
			...
			foreach ( $param ['user_list'] as $p ) {
				...
				if ($uid == 0) { // 新增加的用户
					...
					$uid > 0 && M ( 'apps_follow' )->where ( $map5 )->setField ( 'uid', $uid );
				} else { // 更新的用户
					$userDao->updateInfo ( $uid, $tempArr );
				}
			}
			M ( 'apps_follow' )->where ( $map2 )->setField ( 'syc_status', 1 );
			$this->jump ( U ( 'syc_user' ), '同步用户数据中，请勿关闭' );
		}
		...
		// $openidArr
		// );
		// M ( 'apps_follow' )->where ( $map2 )->setField ( 'has_subscribe', 1 );
		// }
	...
	function syc_user_group() {
		...
		$uids = M ( 'apps_follow' )->where ( $map )->limit ( 100 )->getFields ( 'uid' );
		...
		// exit ();
		M ( 'apps_follow' )->where ( $map2 )->setField ( 'syc_status', 2 );
	...
	function set_remark() {
		...
		if (empty ( $map ['uid'] )) {
			$this->error ( '400421:用户信息出错' );
		}
		
		$param ['remark'] = I ( 'remark' );
		if (empty ( $param ['remark'] )) {
			$this->error ( '400422:备注不能为空' );
		}
		
		$map ['token'] = get_token ();
		
		$info = M ( 'apps_follow' )->where ( $map )->find ();
		if (! $info) {
			$this->error ( '400423:用户信息出错啦' );
		}
		
		$res = M ( 'apps_follow' )->where ( $map )->save ( $param );
		if ($res !== false) { // 同步到微信端
	...
	function clear_score() {
		$token = get_token ();
		$map ['token'] = $token;
		$users = M ( 'apps_follow' )->where ( $map )->getFields ( 'uid' );
		$scoresave ['score'] = 0;
		$userdao = D ( 'Common/User' );
		// 每个用户积分清0
