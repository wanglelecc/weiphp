...
class WapController extends WapBaseController {
	...
	function userCenter() {
		...
		if ($dirs === FALSE || ! file_exists ( ONETHINK_ADDON_PATH )) {
			$this->error ( '400428:插件目录不可读或者不存在' );
		}
		
		$arr = array ();
		$_REQUEST ['doNotInit'] = 1;
		foreach ( $dirs as $d ) {
			if (! file_exists ( ONETHINK_ADDON_PATH . $d . '/Model/WeixinAddonModel.class.php' ))
				continue;
	...
	function check() {
		...
		// 获取微信权限节点
		$map2 ['type_' . $type] = 1;
		$auth = M ( 'apps_auth' )->where ( $map2 )->getFields ( 'name,title' );
