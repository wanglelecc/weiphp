...
class WapController extends WapBaseController {
	...
	function receive() {
		...
		$this->deal ( $info );
		if ($_GET['invite_uid']>0 && $_GET['invite_uid'] != $this->mid){
		    $detail_url = addons_url ( 'Invite://Wap/detail', array (
		        'id' => $info ['id'],
		        'invite_uid' => $_GET['invite_uid'],
		        'token' => get_token ()
		    ) );
		}else{
		    $detail_url = addons_url ( 'Invite://Wap/detail', array (
...
// 			$url = addons_url ( 'WeiSite://WeiSite/lists' );
			...
			$this->error( '400292:本次微邀约还没开始，下次再来吧。', $url );
		}
		if ($data ['num'] <= $data ['collect_count'] || ($data ['end_time'] > 0 && $data ['end_time'] < NOW_TIME)) {
// 			$url = addons_url ( 'WeiSite://WeiSite/lists' );

		    $url = addons_url ( 'Invite://Wap/lists' );
			$this->error( '400293:本次微邀约已经结束，下一次再来吧。', $url );
		}
		...
		if ($info ['experience'] > $user ['experience']) {
			$url = addons_url ( 'Invite://Wap/lists' );
			$this->error( '400294:你的经历值不足', $url );
		}
		
		// 增加领取记录
		$map ['invite_num'] = 0;
		$map ['invite_uid'] = I('invite_uid',0);
...
// 		$this->deal ( $info );
	...
	function deal($info) {
		...
		if (! empty ( $_GET ['invite_uid'] ) && $_GET ['invite_uid'] != $this->mid) { // && $this->mid>0
			$map ['invite_id'] = I ( 'id' );
			$map3['invite_uid'] = $map2 ['uid'] = $map ['uid'] = I ( 'invite_uid' );
			$map3['uid'] = $this->mid;
			$log = M('invite_user')->where($map3)->find();
			if (!$log){
