...
class WapController extends BaseController {
	...
	function card_show() {
		...
		if ($data) {
			...
			$shareUrl = addons_url ( 'WishCard://Wap/card_show', [ 
					'id' => $id 
			] );
			...
			$this->assign ( 'isMake', $isMake ); // 是否制作的
			                                   // dump($shareUrl);
			$template = ONETHINK_ADDON_PATH . 'WishCard/View/Template/' . $data ['template_cate'] . '/' . $data ['template'] . '/index.html';
			$this->assign ( "assetsPath", SITE_URL . '/Addons/WishCard/View/Template/' . $data ['template_cate'] . '/' . $data ['template'] );
