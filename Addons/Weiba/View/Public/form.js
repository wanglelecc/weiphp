	...
	$.ajax({//验证邮箱是否已注册
        ...
        success:function(r){
            if(r.status === true){
				...
				console.log(U('w3g/Public/login'));
				$.ui.showMask(r.info);
				setTimeout(function(){
...
function favorite_weiba(_ts, id, uid, wid){
   ...
   // var url ="{:U('Weiba/Wap/favorite')}";
    $.post(SITE_URL+'/index.php?s=w8/Weiba/Wap/favorite', {'post_id': id, 'post_uid':uid, 'weiba_id':wid}, function(d){
    	//  $.post(url, {'post_id': id, 'post_uid':uid, 'weiba_id':wid}, function(d){
    	if(d==1){
             $(_ts).hide();
             $(_ts).siblings().show();
        }else{
...
function unfavorite_weiba(_ts, id, uid, wid){
   ...
   $.post(SITE_URL+'/index.php?s=w8/Weiba/Wap/unfavorite', {'post_id': id, 'post_uid':uid, 'weiba_id':wid}, function(d){
	   if(d==1){
             $(_ts).hide();
             $(_ts).siblings().show();
        }else{
...
function del_weiba_reply(_ts, rid){
    ...
    $.post(SITE_URL+'/index.php?s=/Weiba/Wap/delReply', {'widget_appname': 'weiba','reply_id': rid}, function(d){
        if(d==1){
