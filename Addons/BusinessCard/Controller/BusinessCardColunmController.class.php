...
class BusinessCardColunmController extends ManageBaseController {
	...
	function add(){
	    ...
	    if (IS_POST) {
	        ...
	        if ($Model->create () && $id = $Model->add ()) {
	            ...
	            $this->error( '400018:'. $Model->getError () );
	        }
	    } else {
	        ...
	        if (empty($business_card_id)){
	            ...
	            if (empty($business_card_id)){
	                $this->error( '400019:请先添加微名片基本信息',addons_url('BusinessCard://BusinessCard/edit'));
	            }
	        }
...
// 	        $fields['business_card_id']['is_show']=4;
	...
	function edit() {
	    ...
	    $data || $this->error( '400020:数据不存在！' );
	
	    // $token = get_token ();
	    // if (isset ( $data ['token'] ) && $token != $data ['token'] && defined ( 'ADDON_PUBLIC_PATH' )) {
	    // $this->error( '400021:非法访问！' );
	    // }
	    ...
	    if (IS_POST) {
	        ...
	        if ($Model->create () && $Model->save ()) {
	            ...
	            $this->error( '400022:'. $Model->getError () );
	        }
	    } else {
	        $fields = get_model_attribute ( $model ['id'] );
	        $this->assign ( 'fields', $fields );
