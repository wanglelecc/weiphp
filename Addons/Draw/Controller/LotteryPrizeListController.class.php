...
class LotteryPrizeListController extends ManageBaseController {
	public function lists() {
		...
		foreach ( $list_data ['list_data'] as &$vo ) {
			...
			$vo ['award_id'] = $vo ['awardarr'] ['name'];
			$vo ['sports_id'] = $vo ['sports'] ['vs_team'];
		}
	...
	function sports_add($sports_id) {
		...
		if (IS_POST) {
			...
			foreach ( $awardNumArr as $id => $num ) {
				...
				if ($awardArr [$id] ['award_type'] == 1) {
					if ($lp [$id] + $awardArr [$id] ['count'] < $num) {
						$this->error( '400223:'. $awardArr [$id] ['name'] . '数量不能超过奖品库剩余的数量！' );
					} else {
			...
			// if ($prizeNum>9){
			// $this->error( '400224:一场比赛只能设置 9 种奖品');
			// }
			foreach ( $awardNumArr as $id => $num ) {
	...
	public function lzwg_add($lzwgid) {
		...
		if (IS_POST) {
			...
			foreach ( $awardNumArr as $id => $num ) {
				if ($num < 0) {
					$this->error( '400225:奖品数量不能小于0' );
				}
				// if ($num>0){
				// $prizeNum++;
				// }
				if ($lp [$id] + $awardArr [$id] ['count'] < $num) {
					$this->error( '400226:'. $awardArr [$id] ['name'] . '数量不能超过奖品库剩余的数量！' );
				} else {
					$awardArr [$id] ['count'] = $awardArr [$id] ['count'] + $lp [$id] - $num;
				}
			}
			// if ($prizeNum>9){
			// $this->error( '400227:一场活动只能设置 9 种奖品');
			// }
			foreach ( $awardNumArr as $id => $num ) {
