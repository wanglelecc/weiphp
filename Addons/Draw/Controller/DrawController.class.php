...
class DrawController extends ManageBaseController {
	...
	function lzwg_activities_lists() {
		...
		foreach ( $list_data ['list_grids'] as &$vo ) {
			if ($vo ['field'] [0] == 'id') {
				$previewUrl = 'Sucai/package?id=[id]&_addons=Sucai&source=Lzwg&is_preview=1&target=_blank|预览';
				$downloadUrl = 'Sucai/package?id=[id]&_addons=Sucai&source=Lzwg&is_download=1|素材下载';
	...
	function lzwg_edit() {
		...
		if (IS_POST) {
			...
			if ($Model->create () && $Model->save ()) {
				...
				$this->error( '400201:'. $Model->getError () );
			}
		} else {
	...
	function lzwg_add() {
		...
		if (IS_POST) {
			...
			if ($Model->create () && $id = $Model->add ()) {
				...
				$this->error( '400202:'. $Model->getError () );
			}
		} else {
			$fields = get_model_attribute ( $model ['id'] );
			$this->assign ( 'fields', $fields );
	...
	function checkTime($startTime, $endTime, $id = 0) {
		if ($endTime < $startTime) {
			$this->error( '400203:结束时间不能大于开始时间！' );
			exit ();
		}
