...
class CommentController extends ManageBaseController {
	function lists() {
...
// 		lastsql();
	...
	function addBlack(){
		...
		if(!empty($postData['value'])){
			...
			$val = $sensitiveStr.','.$postData['value'];
			D('Admin/Config')->setValue ( 'SENSITIVE_WORDS', $val );
			if($res){
				$data['result'] = 'success';
				$data['msg'] = '添加成功!';
				$this -> ajaxReturn($data,'JSON');
	...
	function change_audit(){
		...
		M('comment')->where(['id' => $id ])->setField('is_audit',$data['is_audit']);
		$this->success ( '修改成功' );
	}
}
