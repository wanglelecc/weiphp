      ...
      <?php if($add_button || $del_button || $search_button || !empty($top_more_button)): ?><div class="table-bar">
        ...
        <?php if($search_button): ?><div class="search-form fr cf">
          <div class="sleft" style="margin-right:10px">
           <input style="width:100px;" type="text" name="number" class="search-input" value="<?php echo $_GET['number'];?>" placeholder="请输入会员卡号">
            <a class="sch-btn" href="javascript:;" id="number_search" url="<?php echo U('lists?model='.$model['id']);?>"><i class="btn-search"></i></a> 
           </div>
           <div class="sleft" style="margin-right:10px">
           
            <input style="width:100px;" type="text" name="phone" class="search-input" value="<?php echo $_GET['phone'];?>" placeholder="请输入手机号">
            <a class="sch-btn" href="javascript:;" id="phone_search" url="<?php echo U('lists?model='.$model['id']);?>"><i class="btn-search"></i></a> 
            </div>
           <div class="sleft">
            ...
            <input style="width:100px" type="text" name="<?php echo ($search_key); ?>" class="search-input" value="<?php echo I($search_key);?>" placeholder="<?php echo ($placeholder); ?>">
...
<!--                   <?php if(is_array($list_grids)): $i = 0; $__LIST__ = $list_grids;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$grid): $mod = ($i % 2 );++$i;?>-->
<!--                     <td><?php echo ($data[$key]); ?></td> -->
<!--<?php endforeach; endif; else: echo "" ;endif; ?> -->
							<td><?php echo ($data["draw_id"]); ?></td>
							<td><?php echo ($data["award_id"]); ?></td>
							<td><?php echo ($data["award_name"]); ?></td>
							<td><?php echo ($data["zjtime"]); ?></td>
...
<!-- 							<td><?php echo ($data["remark"]); ?></td> -->
							...
							<td>
								<a onClick="$.WeiPHP.openSubmitDialog('发放奖品','<?php echo U('do_fafang?model='.$model['id'],array('id'=>$data['id']));?>',600,400)" href="javascript:;">发放奖品</a>
							</td>
...
$(function(){
	...
	$("#search,#phone_search,#number_search").click(function(){
        ...
        if(query == '' ){
        	...
        	if(addon){
        		url="<?php echo addons_url(MODULE_NAME.'://'.CONTROLLER_NAME.'/lists');?>&mdm=<?php echo ($_GET['mdm']); ?>";
        	}
        }
	...
	$("input[name='state']").click(function(){
		...
		var awardId = "<?php echo ($_GET['awardId']); ?>";
		location.href="<?php echo U('games_lucky_lists');?>&state="+state+"&awardId="+awardId;
	});
})
</script> 
 <!-- 用于加载js代码 -->
