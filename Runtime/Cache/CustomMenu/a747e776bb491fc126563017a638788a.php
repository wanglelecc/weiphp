...
<?php if(!empty($normal_tips)): ?><p class="normal_tips"><b class="fa fa-info-circle"></b> <?php echo ($normal_tips); ?></p><?php endif; ?>
    <?php if(empty($rule_lists)): ?><div class="null_deal"><p>aOh!<a href="<?php echo U('add_custom_tag');?>">请先增加个性化规则!</a></p>
      <?php else: ?>
			<div class="table-bar">
				<div class="tools">
					<a class="btn" href="<?php echo U('add_custom_tag');?>">增加规则</a>
				</div>
			</div>
      <!-- 规则配置 -->
      <ul class="rule_nav">
        <?php if(is_array($rule_lists)): $i = 0; $__LIST__ = $rule_lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li 
          <?php if($rule_id==$vo['id']) echo 'class="cur"'; ?>
          ><a class="rule" href="<?php echo U('custom_lists',['rule_id'=>$vo['id']]);?>"><?php echo ($vo["title"]); ?></a><a href="javascript:;" rel="<?php echo ($vo["id"]); ?>" class="del delRule">删除</a>
...
#phone {
	...
	width: 400px;
	height: 698px;
	background: url(<?php echo SITE_URL;?>/Public/Home/images/preview_phone_short.jpg) no-repeat center 0;
}
#frame {
	position: absolute;
	left: 0;
...
.wx_menu .keyboard {
	...
	width: 44px;
	height: 50px;
	background: url(/Public/Home/images/wx_menu_keyboard_icon.png) no-repeat;
...
.wx_menu .sub_menu em {
	...
	left: 50%;
	margin-left: -5px;
	background: url(/Public/Home/images/arrow_down.png) no-repeat center bottom;
}
</style>
        ...
        <div class="wx_menu"> <span class="keyboard"></span>
          <div class="menu">
            <?php if(is_array($list_data)): $i = 0; $__LIST__ = $list_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i; if(($data['pid']) == "0"): ?><div class="m_a"  href="javascript:;">
                  <div class="m_a_i"> <img src="/Public/Home/images/wx_menu_list_icon.png"/><?php echo ($data["title"]); ?> </div>
                  <div class="sub_menu">
    ...
    <table cellpadding="0" cellspacing="1">
      ...
      <tbody>
        <?php if(is_array($list_data)): $i = 0; $__LIST__ = $list_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr> 
            <td><input class="ids" type="hidden" value="<?php echo ($data["id"]); ?>" name="ids[]">
              <?php if(($data['pid']) == "0"): ?><strong><?php echo ($data["title"]); ?></strong>
                <?php else: ?>
                &nbsp;&nbsp;&nbsp;&nbsp; ◆ <?php echo ($data["title"]); endif; ?></td>
...
<div class="form-item form_bh" style="margin:20px;">
  <button style="padding:12px 40px;" class="btn ajax-post confirm" type="button" target-form="ids" url="<?php echo U('send_menu',['rule_id'=>$rule_id]);?>"><?php echo ((isset($submit_name) && ($submit_name !== ""))?($submit_name):'发布'); ?></button>
</div>
<script type="text/javascript">
$(function(){
	...
	$('.rule_nav li').hover(function(){
	  ...
	  $('.delRule').click(function(){
			...
			if(confirm("你确认要删除此规则吗？")){
				$.post("<?php echo U('delRule');?>",{id:id},function(){
