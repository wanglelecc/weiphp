...
class App {
	...
	static public function exec() {
		...
		// 插件调用 weiphp 20170411 兼容3.0
		define ( 'ADDON_BASE_PATH', SITE_PATH . '/Addons/' . MODULE_NAME );
		define ( 'ADDON_PUBLIC_PATH', __ROOT__ . '/Addons/' . MODULE_NAME . '/View/Public' );
		...
		if (! preg_match ( '/^[A-Za-z](\/|\w)*$/', $controller_name )) { // 安全检测
			...
			// 创建Action控制器实例
			if (is_dir ( ADDON_PATH . MODULE_NAME )) {
				$module = A ( 'Addons://' . MODULE_NAME . '/' . CONTROLLER_NAME );
			} else {
