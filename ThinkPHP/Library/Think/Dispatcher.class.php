...
class Dispatcher {
	...
	static public function dispatch() {
		...
		if (empty ( $_SERVER ['PATH_INFO'] )) {
			...
			define ( '__INFO__', '' );
			define ( '__EXT__', '' );
			defined ( 'WPID' ) || define ( 'WPID', 0 );
		} else {
			$info = trim ( $_SERVER ['PATH_INFO'], '/' );
			$re = C ( 'URL_ROUTE_RULES' );
			$infoArr = explode ( '/', $info, 2 );
			
			$appid = ltrim ( $infoArr [0], 'w' );
			if (! is_numeric ( $appid )) {
				$appid = 0;
				$infoArr [1] = $info;
			}
			
			defined ( 'WPID' ) || define ( 'WPID', $appid );
			
			define ( '__INFO__', $infoArr [1] );
			// URL后缀
		...
		// 凡星改造 20170411，让路由能直接支持插件
		$path = '';
		is_dir ( APP_PATH . MODULE_NAME ) && $path = APP_PATH . MODULE_NAME;
		empty ( $path ) && is_dir ( ADDON_PATH . MODULE_NAME ) && $path = ADDON_PATH . MODULE_NAME;
		
		// 检测模块是否存在
		if (MODULE_NAME && (defined ( 'BIND_MODULE' ) || ! in_array_case ( MODULE_NAME, C ( 'MODULE_DENY_LIST' ) )) && ! empty ( $path )) {
			// 定义当前模块路径
