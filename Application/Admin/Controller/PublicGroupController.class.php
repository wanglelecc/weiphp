...
class PublicGroupController extends AdminController {
	...
	function PublicGroup() {
		...
		$public_list = M ( 'apps' )->field ( 'group_id,count(id) as public_count' )->group ( 'group_id' )->select ();
		foreach ( $public_list as $p ) {
			$public_count [$p ['group_id']] = $p ['public_count'];
		}
		
		// 获取模型信息
		$model = M('model')->getByName ( 'apps_group' );
	...
	function add() {
		...
		if (IS_POST) {
			...
			if ($Model->create () && $Model->add ()) {
				...
				$this->error( '140214:' . $Model->getError () );
			}
		} else {
	...
	function editPublicGroup() {
		$model = $this->getModel ( 'apps_group' );
		$id = I ( 'id', 0, 'intval' );
		...
		if (IS_POST) {
			...
			if ($Model->create () && $Model->save ()) {
				...
				$this->error( '140215:' . $Model->getError () );
			}
		} else {
			$fields = get_model_attribute ( $model ['id'] );
			$this->_deal_addon ( $fields );
	...
	public function delPublicGroup() {
		...
		if ($Model->where ( $map )->delete ()) {
			$save ['group_id'] = intval ( C ( 'DEFAULT_APPS_GROUP_ID' ) );
			...
			M ( 'apps' )->where ( $map2 )->save ( $save );
			
			$this->success ( '删除成功' );
		} else {
			$this->error( '140218:删除失败！' );
		}
	}
	...
	function PublicAdmin() {
		...
		if ($ids) {
			...
			$link = M ( 'apps_link' )->where ( $map )->select ();
		...
		$this->assign ( 'model', $model );
		$this->meta_title = $model ['title'] . '管理';
		$this->assign ( 'add_url', U('Home/Apps/add', array('from'=>5)) );
		$this->display ( 'Think:lists' );
	}
	function PublicLink() {
	...
	function editPublicAdmin() {
		...
		if (IS_POST) {
			...
			if ($Model->create () && $Model->save ()) {
				...
				$this->error( '140219:' . $Model->getError () );
			}
		} else {
			$fields = get_model_attribute ( $model ['id'] );
