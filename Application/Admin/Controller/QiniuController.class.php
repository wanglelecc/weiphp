...
class QiniuController extends AdminController {
    ...
    public function del(){
        ...
        if($file){
            ...
            if(false === $result){
                $this->error( '140230:' . $this->qiniu->errorStr);
            }else{
                $this->success('删除成功');
            }
        }else{
            $this->error( '140231:错误的文件名');
        }
    }
    ...
    public function rename(){
        ...
        if(false === $result){
            trace($this->qiniu->error);
            $this->error( '140232:' . $this->qiniu->errorStr);
        }else{
            $this->success('改名成功');
        }
    }
    ...
    public function batchDel(){
        ...
        if(is_array($files) && $files !== array()){
            ...
            if(false === $result){
                $this->error( '140233:' . $this->qiniu->errorStr);
            }else{
                $this->success('删除成功');
            }
        }else{
            $this->error( '140234:请至少选择一个文件');
        }
    }
